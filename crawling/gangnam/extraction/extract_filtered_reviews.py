...
review_files = [
    ...
    r'D:\git_gb4pro\crawling\gangnam\data\reviews_year\gangnam_reviews_2026_20260109_232353.csv'
]

# Output paths
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
output_data_file = f'D:\\git_gb4pro\\crawling\\gangnam\\data\\seoul_skin_reviews_2024_2026_{timestamp}.csv'
output_report_file = f'D:\\git_gb4pro\\crawling\\gangnam\\data\\report_review_extraction_{timestamp}.md'
...
def process():
    ...
    for f in review_files:
        ...
        try:
            # Reviews can be large, read in chunks if necessary, but here let's try reading at once 
            # as 2024/2025 files are ~200MB and ~350MB.
            # Use low_memory=False to avoid DtypeWarning
            rdf = pd.read_csv(f, encoding='utf-8-sig', low_memory=False)
            
            # Check hospital_id column. It might be 'hospital_id' or 'id'
...
**5. 특이사항**
*   선정된 645개 병원 ID를 기준으로 병원 ID가 매칭되는 리뷰만 통합하여 저장함.
"""
