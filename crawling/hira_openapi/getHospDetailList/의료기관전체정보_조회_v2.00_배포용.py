"""
의료기관 전체정보 조회 API 호출 스크립트 v2.00 (배포용)
================================================================================
작성일: 2026-01-15
...
# 2. 인증키 인코딩 여부
# 일반적으로 False 사용 (디코딩 키 사용 시)
USE_ENCODED_KEY = False

# 3. 입력 파일 설정 (필수)
# 병원기본목록 CSV 파일의 전체 경로를 입력하세요
# 예: r"C:\Users\사용자명\Documents\병원목록.csv"
INPUT_CSV_FILE = r"여기에_입력_CSV_파일_경로를_입력하세요"

# 4. 요양기호 컬럼명 (선택사항)
# None으로 설정하면 자동으로 탐지합니다 ('ykiho', '암호화요양기호', '요양기호' 등)
YKIHO_COLUMN = None
...
# 7-1. 중간 저장 설정 (진행 중인 데이터를 주기적으로 CSV로 저장)
ENABLE_INTERIM_SAVE = True  # 중간 저장 활성화 (True: 활성화, False: 비활성화)
INTERIM_SAVE_INTERVAL = 10  # 중간 저장 간격 (건수) - 10건마다 CSV 저장

# 8. 테스트 모드 설정
# True로 설정하면 소량 데이터만 처리하여 테스트할 수 있습니다
TEST_MODE = False        # 테스트 모드 (True: 활성화, False: 비활성화)
MAX_TEST_RECORDS = 3     # 테스트 모드에서 처리할 최대 레코드 수

# ============================================================================
# ⚠️ 주의: 아래 코드는 수정하지 마세요
# ============================================================================
...
def main():
    ...
    if SERVICE_KEY == "여기에_발급받은_디코딩_인증키를_입력하세요":
        ...
        print("스크립트 상단의 SERVICE_KEY 변수에 발급받은 인증키를 입력하세요.")
        return
    
    # 입력 파일 확인
    if INPUT_CSV_FILE == "여기에_입력_CSV_파일_경로를_입력하세요":
        print("[오류] 입력 파일 경로를 설정하지 않았습니다.")
        print("스크립트 상단의 INPUT_CSV_FILE 변수에 CSV 파일 경로를 입력하세요.")
        return
