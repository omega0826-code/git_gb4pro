...
SERVICE_KEY = "Bk8LikYxwbpxf1OKF0mYYonK9RNmYo%2FmmgtNsZ41rRNxMuIh5s7RgflEXp%2BXwp3R0FDR2j01gx62Hc%2B%2BJzc2pw%3D%3D"
...
def main():
    ...
    try:
        ...
        if hospitals:
            ...
            # 파일명 생성 (현재 날짜시간 포함)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            csv_filename = output_dir / f"서울_강남구_피부과_{timestamp}.csv"
            
            # CSV 저장 (암호화된 요양기호 포함)
            save_to_csv(hospitals, str(csv_filename), include_ykiho=True)
