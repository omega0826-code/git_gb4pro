...
print("=" * 80)
print("병원명 기준 재매칭 및 강남언니 입점 비교 분석")
print("=" * 80)
...
# 전체 병원 데이터
전체_df = pd.read_csv(
    r'd:\git_gb4pro\crawling\openapi\getHospDetailList\data\병원전체정보_결측치처리완료_20260119_233319.csv',
    encoding='utf-8-sig'
)
print(f"  - 전체 병원 데이터: {len(전체_df):,}건")

# 강남언니 데이터
강남언니_df = pd.read_csv(
    r'd:\git_gb4pro\crawling\gangnam\data\archive\gangnam_hospitals_detail_RESUME_20260105_041227.csv',
    encoding='utf-8-sig'
)
print(f"  - 강남언니 데이터: {len(강남언니_df):,}건")
...
# 강남언니 데이터의 병원명 컬럼 확인
if 'hospital_name' in 강남언니_df.columns:
    강남언니_df['매칭_병원명'] = 강남언니_df['hospital_name'].str.strip().str.replace(' ', '')
elif 'yadmNm' in 강남언니_df.columns:
    강남언니_df['매칭_병원명'] = 강남언니_df['yadmNm'].str.strip().str.replace(' ', '')
else:
    # 첫 번째 병원명 관련 컬럼 사용
    name_cols = [c for c in 강남언니_df.columns if '명' in c or 'name' in c.lower()]
    if name_cols:
        강남언니_df['매칭_병원명'] = 강남언니_df[name_cols[0]].str.strip().str.replace(' ', '')

# 입점 병원명 리스트
입점_병원명_set = set(강남언니_df['매칭_병원명'].dropna().unique())
print(f"  - 강남언니 입점 병원명: {len(입점_병원명_set)}개")

# 입점 여부 판단
전체_df['입점여부'] = 전체_df['매칭_병원명'].apply(
    lambda x: '입점' if x in 입점_병원명_set else '미입점'
)

입점_count = (전체_df['입점여부'] == '입점').sum()
미입점_count = (전체_df['입점여부'] == '미입점').sum()

print(f"\n  [매칭 결과]")
print(f"    입점: {입점_count:,}건 ({입점_count/len(전체_df)*100:.1f}%)")
print(f"    미입점: {미입점_count:,}건 ({미입점_count/len(전체_df)*100:.1f}%)")
...
# 의사 유형
전체_df['의사유형'] = 전체_df['dgsbjt_dgsbjtPrSdrCnt'].apply(
    lambda x: '전문의' if x > 0 else '일반의'
)

# 병원 규모
전체_df['총의사수'] = 전체_df['dgsbjt_cdiagDrCnt'] + 전체_df['dgsbjt_dgsbjtPrSdrCnt']
전체_df['병원규모'] = 전체_df['총의사수'].apply(
    lambda x: 'Type A (대형)' if x >= 5 
    else ('Type B (중형)' if x >= 2 else 'Type C (1인)')
)

# 수술 가능 여부
전체_df['수술가능'] = 전체_df['eqp_soprmCnt'].apply(
...
# 그룹 분리
입점_df = 전체_df[전체_df['입점여부'] == '입점'].copy()
미입점_df = 전체_df[전체_df['입점여부'] == '미입점'].copy()
...
# 5-4. 행정동별 입점률
행정동별_입점률 = 전체_df.groupby('eqp_emdongNm').apply(
    lambda x: (x['입점여부'] == '입점').sum() / len(x) * 100
).sort_values(ascending=False)

print(f"\n  [행정동별 입점률 TOP 5]")
for i, (dong, rate) in enumerate(행정동별_입점률.head(5).items(), 1):
    total = len(전체_df[전체_df['eqp_emdongNm'] == dong])
    입점 = (전체_df[전체_df['eqp_emdongNm'] == dong]['입점여부'] == '입점').sum()
...
# 병원 규모 분포
ax1 = axes[0, 0]
categories = ['Type C (1인)', 'Type B (중형)', 'Type A (대형)']
x = np.arange(len(categories))
width = 0.35

입점_규모 = [입점_병원규모.get(cat, 0) for cat in categories]
미입점_규모 = [미입점_df['병원규모'].value_counts().get(cat, 0) for cat in categories]

ax1.bar(x - width/2, 입점_규모, width, label='입점', color='#4A90E2')
ax1.bar(x + width/2, 미입점_규모, width, label='미입점', color='#E74C3C')
...
# 전문의 비율
ax2 = axes[0, 1]
doctor_categories = ['전문의', '일반의']
입점_전문의 = [입점_의사유형.get('전문의', 0), 입점_의사유형.get('일반의', 0)]
미입점_전문의 = [미입점_df['의사유형'].value_counts().get('전문의', 0),
                 미입점_df['의사유형'].value_counts().get('일반의', 0)]

x2 = np.arange(len(doctor_categories))
ax2.bar(x2 - width/2, 입점_전문의, width, label='입점', color='#4A90E2')
ax2.bar(x2 + width/2, 미입점_전문의, width, label='미입점', color='#E74C3C')
...
ax2.set_xticks(x2)
ax2.set_xticklabels(doctor_categories)
ax2.legend()
...
analysis_summary = {
    ...
    'data_overview': {
        ...
        '입점률': float(입점_count / len(전체_df) * 100)
    },
    'part1_입점업체특징': {
        '기본_통계': 입점_통계,
...
# 입점 여부 포함 데이터 저장
output_csv = f'{output_dir}/병원데이터_입점여부포함_{timestamp}.csv'
전체_df.to_csv(output_csv, index=False, encoding='utf-8-sig')
print(f"  - 저장: 병원데이터_입점여부포함_{timestamp}.csv")
...
print(f"  4. 병원데이터_입점여부포함_{timestamp}.csv")
print(f"\n저장 위치: {output_dir}")
