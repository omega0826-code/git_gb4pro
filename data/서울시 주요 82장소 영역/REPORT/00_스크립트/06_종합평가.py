...
try:
    df_comp = pd.read_csv(REPORT_BASE / "01_경쟁환경분석" / "상권별_경쟁분포_현황.csv", encoding='utf-8-sig')
    df_cust = pd.read_csv(REPORT_BASE / "02_고객분석" / "상권별_매출현황.csv", encoding='utf-8-sig')
    df_pop = pd.read_csv(REPORT_BASE / "03_인구유동분석" / "상권별_인구현황.csv", encoding='utf-8-sig')
    df_loc = pd.read_csv(REPORT_BASE / "04_입지조건분석" / "상권별_입지조건.csv", encoding='utf-8-sig')
...
df_pop.columns = ['상권_명', '상주인구', '직장인구']
df_pop['인구지표'] = df_pop['상주인구'] + df_pop['직장인구']
df_loc.columns = ['상권_명', '입지지표']

# 데이터 병합
df_merge = df_comp.merge(df_cust, on='상권_명').merge(df_pop[['상권_명', '인구지표']], on='상권_명').merge(df_loc, on='상권_명')
...
sns.barplot(x='종합점수', y='상권_명', data=df_merge, hue='상권_명', palette='magma', legend=False)
...
def create_radar_chart(df, output_path):
    ...
    for i, (idx, row) in enumerate(df.head(3).iterrows()):
        ...
        values += values[:1]
        ax.plot(angles, values, linewidth=2, linestyle='solid', label=row['상권_명'], color=colors[i])
        ax.fill(angles, values, alpha=0.1, color=colors[i])
        
    plt.xticks(angles[:-1], categories)
    plt.legend(loc='lower center', bbox_to_anchor=(0.5, -0.1), ncol=3)
    plt.title('최적 입지 후보 TOP 3 분석 비교', size=16, y=1.1, fontweight='bold')
