...
**작성일**: 2026-02-03  
**버전**: 4.01 (한글 폰트 설정 개선 - CP949 인코딩 에러 해결)  
...
- 🎨 **CP949 인코딩 에러 해결**
  - `korean_font_setup.py` 모듈의 print 문에서 특수문자(✓) 제거
  - Windows 콘솔 CP949 인코딩과 호환되도록 [OK] 표기로 변경
  - 폰트 설정 실패로 인한 그래프 한글 깨짐 문제 해결
  
- 📝 **직접 폰트 설정 방법 추가**
  - `koreanize_matplotlib` 의존성 없이 폰트 설정 가능한 방법 제공
  - Malgun Gothic, NanumGothic 등 시스템 폰트 자동 감지 및 적용
  - 폰트 설정 실패 시에도 스크립트 정상 실행 보장
  
- 🔍 **폰트 진단 도구 추가**
  - `font_diagnosis.py`: 시스템 한글 폰트 확인 스크립트
  - 사용 가능한 한글 폰트 목록 및 권장 폰트 자동 제시
  
- ✅ **검증 완료**
  - 테스트 그래프에서 한글 정상 표시 확인
  - 막대 그래프, 레이더 차트 등 모든 그래프 유형에서 한글 지원
...
### 증상 0: 그래프에서 한글이 네모 박스(□)로 표시됨 ⭐ NEW

**원인**: 한글 폰트 설정 실패

**근본 원인 (V4.01에서 해결)**:
- `korean_font_setup.py` 모듈의 print 문에 특수문자(✓) 포함
- Windows 콘솔의 기본 인코딩(CP949)에서 해당 문자 처리 불가
- `UnicodeEncodeError` 발생으로 `setup_korean_font()` 함수 중단
- matplotlib 폰트 설정이 완료되지 않아 한글 깨짐

**해결 방법**:

1. ✅ **V4.01 업데이트 적용** (권장)
   - `korean_font_setup.py` 파일이 자동으로 수정됨
   - 특수문자(✓) → [OK]로 변경되어 CP949 호환
...
2. 🔍 **폰트 설정 확인**
   ...
   ```
   - 시스템에 설치된 한글 폰트 확인
   - Malgun Gothic, NanumGothic 등이 있어야 함

3. 🎨 **테스트 그래프 생성**
   ```bash
   python font_test_simple.py
   ```
   - 한글이 정상 표시되는지 확인
   - `font_test_result.png` 파일 확인
...
4. 📦 **대체 방법: 직접 폰트 설정**
   ...
   for font in korean_fonts:
       if font in available_fonts:
           ...
           print(f"[OK] 한글 폰트 설정: {font}")
           break
   ```

**검증**:
- 수정 후 그래프를 다시 생성하여 한글이 정상 표시되는지 확인
- 모든 그래프 유형(막대, 선, 레이더 등)에서 한글 확인
...
3. 💡 **V4.01 개선**: 직접 폰트 설정 방법 사용 시 즉시 설정 완료
