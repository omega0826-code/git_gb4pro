...
# 형태소 분석 함수 (Kiwi 최신 API 대응)
def extract_nouns(text):
    results = kiwi.analyze(text)
    nouns = []
    for result in results: # result: Sentence
        for token in result.tokens: # token: Token
            if token.tag.startswith('N') or token.tag == 'XR': # 명사, 어근
                if len(token.form) > 1 and token.form not in STOPWORDS: # 1글자 제외, 불용어 제외
                    nouns.append(token.form)
    return nouns

# 전체 텍스트 분석
write_report("형태소 분석 진행 중... (시간이 소요될 수 있습니다)")
all_nouns = []
for text in intro_df['introduction']:
    try:
        all_nouns.extend(extract_nouns(text))
    except:
        continue # 에러 발생 시 해당 텍스트 건너뜀
...
write_report("\n## 4. 인사이트 요약")
write_report("- 상위 키워드를 통해 강남 병원들이 주로 강조하는 가치를 파악할 수 있음.")
write_report("- 글자 수와 가독성을 고려하여 너무 짧거나 긴 소개글은 개선이 필요할 수 있음.")
