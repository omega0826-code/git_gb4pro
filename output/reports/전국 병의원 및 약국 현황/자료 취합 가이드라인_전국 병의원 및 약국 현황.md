...
전국 병의원 및 약국 현황 데이터는 **12개의 Excel 파일**로 구성되어 있으며, 크게 **3개 카테고리**로 분류됩니다:
...
| 파일명 | 파일 크기 | 예상 레코드 수 |
|--------|----------|--------------|
| 1.병원정보서비스 | 16.2 MB | 대규모 |
| 2.약국정보서비스 | 3.7 MB | 중규모 |
| 3.시설정보 | 19.1 MB | 대규모 |
| 4.세부정보 | 4.5 MB | 중규모 |
| 5.진료과목정보 | 14.1 MB | 대규모 |
| 6.교통정보 | 2.1 MB | 중규모 |
| 7.의료장비정보 | 2.3 MB | 중규모 |
| 8.식대가산정보 | 636 KB | 소규모 |
| 9.간호등급정보 | 438 KB | 소규모 |
| 10.특수진료정보 | 2.0 MB | 중규모 |
| 11.전문병원지정분야 | 20 KB | 소규모 |
| 12.기타인력정보 | 1.7 MB | 중규모 |
...
```python
# 예시 코드
df_hospital = pd.read_excel('1.병원정보서비스(2025.12.).xlsx')
df_facility = pd.read_excel('3.의료기관별상세정보서비스_01_시설정보 2025.12..xlsx')
...
```python
# 예시: 진료과목정보 - 과목 수 집계
df_subject = pd.read_excel('5.의료기관별상세정보서비스_03_진료과목정보 2025.12..xlsx')
...
df_equipment = pd.read_excel('7.의료기관별상세정보서비스_05_의료장비정보 2025.12..xlsx')
...
df_traffic = pd.read_excel('6.의료기관별상세정보서비스_04_교통정보 2025.12..xlsx')
...
# 파일 경로 설정
data_dir = 'd:/git_gb4pro/data/전국 병의원 및 약국 현황 2025.12/'
...
| 항목 | 내용 |
|-----|------|
| **총 파일 수** | 12개 (병원 1 + 약국 1 + 상세 10) |
| **핵심 병합 키** | `암호화요양기호` |
| **병합 전략** | 1:1 (Left Join) + 1:N (Aggregate/Pivot) |
| **주의사항** | 중복 컬럼, 1:N 관계, 메모리 최적화 |
| **최종 산출물** | 통합 CSV/Excel (병원 기준) + 약국 별도 |
